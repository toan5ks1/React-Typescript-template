...
import { HasuraError } from "src/squads/calendar/internals/hasura-client/errors";
import {
    injectCatchResponseInterceptor,
    injectRefreshTokenInterceptor,
    injectMetadataInterceptor,
} from "src/squads/calendar/internals/hasura-client/interceptors";
import reactiveStorage from "src/squads/calendar/internals/reactive-storage";
import { getFakeLocalUser } from "src/squads/calendar/test-utils/mocks/user";
...
const fakeUser = getFakeLocalUser();
...
jest.mock("src/squads/calendar/internals/reactive-storage", () => {
    return {
        get: jest.fn(),
    };
});
