...
function preventWarningMessage() {
    ...
    function makeTestFailed(errType: "warn" | "error", ...msg: any[]) {
        ...
        const exceptions: string[] = [
            ...
            "ra.message.manabie-error.invalid_params", // any error with this message will be ignored
        ...
        if (!shouldIgnoreThisErr) {
            const msgFormated = formatMessage(errType, ...msg);
            messagesWillThrow = messagesWillThrow.concat(msgFormated);
        }
    }

    function throwMessages() {
        if (messagesWillThrow) throw new Error(messagesWillThrow);
    }

    beforeEach(() => {
        messagesWillThrow = ""; //reset afterEach catch multi errors
