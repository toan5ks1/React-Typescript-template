...
import { SyllabusRules } from "./rules";
...
async function defineAbilityFor(
    ability: Ability<AbilityType<SyllabusRules>>,
    ...
    const allRolesPermission = import.meta.globEager<{
        default: RawRuleOf<Ability<AbilityType<SyllabusRules>>>[];
    }>("./variants/**/*.ts");
...
export type AppPermission = ManaPermission<UserGroupKeys, SyllabusRules>;

const permission: AppPermission = new ManaPermission<UserGroupKeys, SyllabusRules>(
    defineAbilityFor
);
