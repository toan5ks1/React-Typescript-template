...
import { AssignmentFormValues } from "src/squads/syllabus/pages/Assignment/common/types";

export interface UseUpsertAssignmentProps {
    action: ProviderTypes.CREATE | ProviderTypes.UPDATE;
    searchUrl: string;
}

export interface UseUpsertAssignmentReturn {
    upsertAssignment: (
...
const useUpsertAssignment = (props: UseUpsertAssignmentProps) => {
    ...
    const { mutate, isLoading } = inferMutation({
        ...
        onSuccess: ({ assignmentIdsList }) => {
            const message = t(
                ...
                { smart_count: t(`resources.${EurekaEntities.ASSIGNMENTS}.assignment`) },
                { lowercase: true }
            );

            showSnackbar(message, "success");
            navigation.push(
                `/${MicroFrontendTypes.SYLLABUS}/${EurekaEntities.ASSIGNMENTS}/${assignmentIdsList[0]}/show${searchUrl}`
            );
        },
        onError: (error) => {
    ...
    const makePayload = useCallback(
        (formData: AssignmentFormValues): NsAssignmentEureka.UpsertAssignment => {
            const {
                assignment_id,
                content,
                display_order,
    ...
    const upsertAssignment = useCallback(
        (formData: AssignmentFormValues, options: Parameters<typeof mutate>[1]) => {
            if (isLoading) return;
...
export default useUpsertAssignment;
