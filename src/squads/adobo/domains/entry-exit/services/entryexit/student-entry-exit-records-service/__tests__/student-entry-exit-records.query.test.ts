...
import studentEntryExitMgmtQueries from "../student-entryexit-records-service-entry-exit.query";
...
describe("student-entry-exit-service-entryexitmgmt", () => {
    it("getMany method", async () => {
        ...
        const _callSpy = jest.spyOn(studentEntryExitMgmtQueries, "_call");
        const result = await studentEntryExitMgmtQueries.getMany({});
    ...
    it("getList method", async () => {
        ...
        const _callSpy = jest.spyOn(studentEntryExitMgmtQueries, "_call");
        const result = await studentEntryExitMgmtQueries.getList({
            student_id: "id01",
        });
        expect(_callSpy).toHaveBeenCalledTimes(1);
    ...
    it("entryExitGetManyStudentRecordsByDate method", async () => {
        ...
        const _callSpy = jest.spyOn(studentEntryExitMgmtQueries, "_call");
        const result = await studentEntryExitMgmtQueries.entryExitGetManyStudentRecordsByDate({
            student_id: "id01",
