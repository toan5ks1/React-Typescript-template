...
export const selectAutocompleteOptionByLabel = (optionName: string) => {
    const options = screen.getByTestId("AutocompleteBase__listBox");
    userEvent.click(within(options).getByText(optionName));
};

export const getAutocompleteInputByTestId = (autocompleteTestId: string) => {
    try {
        ...
        throw new Error(
            `There is no combobox role in [data-testid="${autocompleteTestId}"]. ${error}`
        );
    }
};

export const getRadioInputByTestId = (radioTestId: string) => {
    try {
        const radio = screen.getByTestId(radioTestId);
        const radioInput = within(radio).getByRole("radio");

        return radioInput;
    } catch (error) {
        throw new Error(`There is no radio role in [data-testid="${radioTestId}"]. ${error}`);
    }
};
