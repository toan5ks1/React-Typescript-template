...
import { UnpackNestedValue, DeepPartial, UseFormReturn, RegisterOptions } from "react-hook-form";
import {
...
export interface UseFormFilterAdvancedProps<T> extends UseFormType<T> {
    isNoData: boolean;
    defaultValue: UnpackNestedValue<DeepPartial<T>>;
    filterFieldObjects: FilterAppliedObjectsMap<T>;
    onApplySubmit: (value: T) => void;
    filterAppliedFieldObjects?: FilterAppliedObjectValuesMap<T>;
    onSetFilterAppliedObjects?: (value: FilterAppliedObjectValuesMap<T>) => void;
}

const useFormFilterAdvanced = <T extends Record<string, T[keyof T]>>(
    ...
    const [filterApplied, setFilterApplied] =
        useState<FilterAppliedObjectValuesMap<T>>(filterAppliedFieldObjects);
    ...
    const handleApplyFilter = useCallback(
        (data: UnpackNestedValue<DeepPartial<T>>) => {
            ...
            if (isNoData) {
                ...
                // Return warning for user
                showSnackbar(t("ra.message.invalidSearchQueryReturningAllItems"), "info");
                return;
            }
